...
if (isDemoSeed) {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    bp.push({ id: "b" + i, systolic: 118 + ((i * 3) % 12), diastolic: 74 + ((i * 2) % 8), heartRate: 70 + (i % 10), period: "Morning", date: d.toISOString().slice(0, 10), time: "07:30" });
  }
  const appts = [
  ...
  // Write synchronously via localStorage so initStore() picks them up immediately
  // Capacitor Preferences on web stores under "CapacitorStorage." prefix
  const PFX = "CapacitorStorage.";
  localStorage.setItem(PFX + "dawaa_medications", JSON.stringify(meds));
  localStorage.setItem(PFX + "dawaa_readings", JSON.stringify(bp));
  localStorage.setItem(PFX + "dawaa_appointments", JSON.stringify(appts));
  localStorage.setItem(PFX + "dawaa_labTests", JSON.stringify(labs));
  localStorage.setItem(PFX + "dawaa_doseRecords", JSON.stringify(doses));
  localStorage.setItem(PFX + "dawaa_settings", JSON.stringify(settings));
}
